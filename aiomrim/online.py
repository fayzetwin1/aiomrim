...
async def get_status(url: str, user: str,):
    timeout = aiohttp.ClientTimeout(total=5)
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(f"{url}/users/status", json={"user": user}) as response:
                response.raise_for_status()
                data = await response.text()
                return data 
    except asyncio.TimeoutError:
        ...
        raise APIConnectionError(f"Connection error to url {url}/users/status: {e}")
    except aiohttp.ClientResponseError as e:
        raise APIConnectionError(f"HTTP error {e.status} for url {url}/users/status: {e.message}")
    except aiohttp.ClientError as e:
        raise APIConnectionError(f"Client error for url {url}/users/status: {e}")
    return None